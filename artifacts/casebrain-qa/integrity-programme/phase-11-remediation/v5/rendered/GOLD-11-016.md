# GOLD-11-016 — solicitor-visible render (v5)

> Solicitor-facing text only. No stratum, selection reason, review focus, hypothesis, or prediction metadata.
> Blocked ≠ repaired. Human judgments belong only in the blank workbook.
> Review eligibility: **substantive**

## Controlled source context

Source evidence: controlled redacted context included

```text
Controlled evidence reference: CASE-05:demo-audit-05-encro-attribution
Controlled source context (safely redacted for review)
Client label: Liam Craft
Allegation: Being concerned in the supply of a controlled drug of Class A, namely cocaine
Evidence states:
• Encro message extracts LC/MSG/01 — Served
• Platform / source extraction — Referred only
• Handle attribution report — Missing
• Subscriber / account data — Missing
• Device continuity — Missing
• Co-defendant material — Missing
Chase / outstanding items:
• Platform / source extraction
• Handle attribution report
• Subscriber / account data
• Device continuity
• Full download / export
```

## Offence family (solicitor)

Source evidence: controlled redacted context included

```text
Offence family resolved for this matter.
```

## Case header

Source evidence: controlled redacted context included

```text
Client: Liam Craft
Allegation: Being concerned in the supply of a controlled drug of Class A, namely cocaine
```

## Court line

Source evidence: controlled redacted context included

```text
Item: Disclosure chase request (1)
Status: Copy unavailable
Reason: Legally sensitive formula — queued for qualified solicitor review.
```

## Client-safe summary

Source evidence: controlled redacted context included

```text
Item: Client-safe summary wording
Status: Copy unavailable
Reason: Wrong offence-family wording was blocked.
```

## CPS chase — Platform / source extraction

Source evidence: controlled redacted context included

```text
Please provide Platform / source extraction (see MG6C/ENC/02) or confirm in writing why it is not available.
```

## CPS chase — Handle attribution report

Source evidence: controlled redacted context included

```text
Item: CPS chase — Handle attribution report
Status: Copy unavailable
Reason: Wrong offence-family wording was blocked.
```

## CPS chase — Subscriber / account data

Source evidence: controlled redacted context included

```text
Item: CPS chase — Subscriber / account data
Status: Copy unavailable
Reason: Wrong offence-family wording was blocked.
```

## CPS chase — Device continuity

Source evidence: controlled redacted context included

```text
Please provide Device continuity (see MG6C/ENC/05) or confirm in writing why it is not available.
```

## Evidence truth map

Source evidence: controlled redacted context included

```text
• Encro message extracts LC/MSG/01 — Served
• Platform / source extraction — Referred only
• Handle attribution report — Missing
• Subscriber / account data — Missing
• Device continuity — Missing
• Co-defendant material — Missing
```

## Do-not-overstate warnings

Source evidence: controlled redacted context included

```text
• handle proves defendant
• NIGHTLINE-77 is Liam Craft
• phone proves role
• Do not treat handle or phone reference as proof of the defendant role without served attribution.
```
