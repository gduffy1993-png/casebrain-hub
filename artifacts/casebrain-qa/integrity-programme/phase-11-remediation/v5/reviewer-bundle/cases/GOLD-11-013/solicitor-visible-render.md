# GOLD-11-013 — solicitor-visible render (v5)

> Solicitor-facing text only. No stratum, selection reason, review focus, hypothesis, or prediction metadata.
> Blocked ≠ repaired. Human judgments belong only in the blank workbook.
> Review eligibility: **substantive**

## Controlled source context

Source evidence: controlled redacted context included

```text
Controlled evidence reference: demo-audit-03-bwv-custody
Controlled source context (safely redacted for review)
Client label: Client
Allegation: DA-03 Casey Fry — BWV referred / custody extract only
Hearing: Hearing date not safely extracted
Evidence states:
• Charge sheet — Served
• Officer MG11 — Incomplete
• Custody record extract — Incomplete
• Body-worn video — Referred only
• Full custody record — Missing
• Interview audio — Missing
• Interview transcript — Missing
• PACE safeguards detail — Missing
Chase / outstanding items:
• Full BWV export
• Full custody record
• Interview audio
• Interview transcript
• PACE safeguards detail
```

## Overview evidence counts

Source evidence: controlled redacted context included

```text
Served 3 · Referred 1 · Missing 4 · Incomplete 0 · Not safely confirmed 2
```

## Hearing status

Source evidence: controlled redacted context included

```text
Hearing date not safely extracted
```

## Offence family (solicitor)

Source evidence: controlled redacted context included

```text
Offence family resolved for this matter.
```

## Disclosure chase list

Source evidence: controlled redacted context included

```text
Total 3
• Body-worn video (BWV)
• Interview recording / transcript
• Officer MG11
```

## Copy preview

Source evidence: controlled redacted context included

```text
DA-03 Casey Fry — BWV referred / custody extract only
```

## Copy preview

Source evidence: controlled redacted context included

```text
assault_emergency_worker
```

## Copy preview

Source evidence: controlled redacted context included

```text
Assault an emergency worker, contrary to section 1 of the Assaults on Emergency Workers (Offences) Act 2018
```

## Copy preview

Source evidence: controlled redacted context included

```text
provisional_check_source
```

## Cross-family containment probe

Source evidence: controlled redacted context included

```text
Item: Cross-family containment probe (1)
Status: Copy unavailable
Reason: Wrong offence-family wording was blocked.
```
