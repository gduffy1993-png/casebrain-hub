# GOLD-11-024 — solicitor-visible render (v4)
...
```text
Controlled evidence reference: SYN-RR-01
...
Probe design: Controlled review-required neutral banner path
Controlled input construction: Same controlled raw-fragment omission probe design as the paired overview-field probe, checked on the review-required surface instead.
Expected behaviour under test: Solicitor sees the review-required neutral message; the output must not be treated as repaired prose.
```
