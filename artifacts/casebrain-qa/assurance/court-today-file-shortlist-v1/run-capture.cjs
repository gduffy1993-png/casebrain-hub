/**
 * Live PR #101 preview capture: Patterson, Davies, Trap fresh, Brookes fresh.
 * Court Today + Overview + Chase. Gold20 only. Does not write the password into artefacts.
 *
 *   node artifacts/casebrain-qa/assurance/court-today-file-shortlist-v1/run-capture.cjs
 */
...
const TABS = [
  { key: "overview", extraOk: null },
  { key: "disclosure-chase", extraOk: (t) => /chase|outstanding|disclosure/i.test(t) },
  { key: "today", extraOk: (t) => /Top chase|Safe court line|Before court|Our read/i.test(t) },
];
...
(async () => {
  ...
  const report = { signedIn: false, recaptured: [], errors: [], base: BASE, email: EMAIL };
  try {
    ...
    for (const spec of CASES) {
      ...
      for (const tab of TABS) {
        const url = `${BASE}/cases/${spec.caseId}?tab=${tab.key}&controlRoom=1&demoShell=1`;
