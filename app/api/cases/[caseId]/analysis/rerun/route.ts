 ...
 * 
 * Re-runs analysis for a case by selecting all documents and calling rebuild logic
 * This is a convenience endpoint that automatically includes all case documents
 */
  ...
  try {
    ...
    try {
      try {
        ...
        console.error("[rerun-analysis] Failed to get user email:", userError);
      ...
      console.error("[rerun-analysis] Trial status check failed, allowing request:", trialError);
      // Continue with the rerun - better to allow than crash
    }
    ...
    // Get ALL documents for this case (rerun includes everything)
    ...
    if (docsError) {
      console.error("[rerun-analysis] Error fetching documents:", docsError);
      return NextResponse.json(
    ...
    try {
      ...
      console.warn("[rerun-analysis] Failed to detect case role, defaulting to claimant:", error);
      caseRole = "claimant";
    }
    ...
    try {
      ...
      console.error("[rerun-analysis] Failed to generate move sequence:", error);
    ...
    if (insertError) {
      console.error("[rerun-analysis] Failed to insert version:", insertError);
      return NextResponse.json(
    ...
    // Log rerun
    console.log(`[rerun-analysis] Created version ${nextVersionNumber} for case ${caseId}, momentum: ${momentum.state}, documents: ${document_ids.length}`);
    ...
    return NextResponse.json({
      ...
      message: `Analysis re-run successfully. Created version ${nextVersionNumber}.`,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "Failed to re-run analysis", message: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
}
