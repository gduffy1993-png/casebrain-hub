#!/usr/bin/env npx tsx
/**
 * Messy PDF-backed proof pack + proof receipts (v3 scale-100).
 *
 * Run:
 *   npx tsx scripts/run-messy-pdf-proof-v3-scale100.ts
 */
...
const OUT_ROOT = path.join(ROOT, "artifacts", "casebrain-qa", "messy-pdf-proof-v3-scale100");
...
function buildScenarios(): ScenarioSpec[] {
  ...
  for (const sourceCaseId of EXPANSION_SOURCE_IDS) {
    const meta = SOURCE_META[sourceCaseId];
    for (const variant of EXPANSION_VARIANTS) {
      const caseId = `messy-pdf-v3-${String(index).padStart(3, "0")}-${meta.family}-${variant.slug}`.toLowerCase();
      scenarios.push({
        index,
        sourceCaseId,
        caseId,
      ...
      index += 1;
      if (scenarios.length === 100) return scenarios;
    }
  }
...
function writeWorstIssues(runs: CaseRun[]) {
  ...
  const lines = [
    "# WORST ISSUES — messy-pdf-proof-v3-scale100",
    "",
    "Top 20 for warning pressure and readability focus.",
    "",
    "| Case | FAIL | WARNING | Hard issue hits | Soft warning hits | Top soft drivers |",
    "|------|-----:|--------:|----------------:|------------------:|------------------|",
    ...worst.slice(0, 20).map((w) => {
      const run = runs.find((r) => r.spec.caseId === w.caseId)!;
      const rankedSoft = Object.entries(run.softWarnings)
        .sort((a, b) => b[1] - a[1])
        .filter(([, n]) => n > 0)
        .slice(0, 3)
        .map(([k, n]) => `${k}:${n}`)
        .join(", ");
      return `| ${w.caseId} | ${w.fail} | ${w.warning} | ${w.hardTotal} | ${w.softTotal} | ${rankedSoft || "none"} |`;
    }),
    "",
  ];
...
function writeRepeatedPatterns(runs: CaseRun[]) {
  ...
  const top = [...patterns.entries()].filter(([, n]) => n >= 8).sort((a, b) => b[1] - a[1]).slice(0, 30);
  const md = [
    "# REPEATED PATTERNS — messy-pdf-proof-v3-scale100",
    "",
    "| Pattern | Hits |",
    "|---------|-----:|",
...
function writePackSummary(runs: CaseRun[]) {
  ...
  const md = [
    ...
    "## Acceptance checks",
    "",
    `- 100/100 build: **${totals.casesRun === 100 ? "yes" : "no"}**`,
