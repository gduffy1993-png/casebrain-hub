/**
 * Court criminal corpus sweep — Chunk C0 find-only (Court Control Room + Chase court-lines).
 *
 * Projects Disclosure Chase / safe court line / hearing-war-room court claims vs PDF/source text.
 * Read-only w.r.t product behaviour / DB (service-role READ only).
 * Routes:
 *   BACKEND_LIVE              — reuse extracted_text already on eval/QA cases
 *   OFFLINE_COURT_PROJECTION  — extract PDF text + Court claim projectors
 *   SKIP                      — no text, corrupt, non-criminal, duplicate hash
 *
 * Resume:
 *   npx tsx scripts/assurance/court-criminal-sweep/run-court-criminal-sweep.ts
 *   npx tsx scripts/assurance/court-criminal-sweep/run-court-criminal-sweep.ts --limit=50
 *   npx tsx scripts/assurance/court-criminal-sweep/run-court-criminal-sweep.ts --concurrency=6
 *
 * Reuse criminal unique index:
 *   COURT_SWEEP_REUSE_INDEX=1 COURT_SWEEP_INDEX_SRC=artifacts/.../papers-criminal-sweep-v1/CRIMINAL-UNIQUE-INDEX.csv
 */
...
const PRODUCT_SHA = process.env.F167_PRODUCT_SHA?.trim() || "e3179fa74";
const PREVIEW =
  process.env.F167_PREVIEW?.replace(/\/$/, "") ||
  "https://casebrain-76gk8vbwk-gduffy1993-pngs-projects.vercel.app";
const DEFAULT_OUT = path.join(ROOT, "artifacts/casebrain-qa/assurance/court-criminal-sweep-v1");
const OUT_DIR = (() => {
  const raw = process.env.COURT_SWEEP_OUT_DIR?.trim();
...
const NDJSON = path.join(OUT_DIR, "court-sweep.ndjson");
const HITLIST_CSV = path.join(OUT_DIR, "COURT-FAIL-HITLIST.csv");
const STATUS_MD = path.join(OUT_DIR, "COURT-SWEEP-STATUS.md");
...
const REUSE_INDEX =
  process.env.COURT_SWEEP_REUSE_INDEX?.trim() === "1" ||
  Boolean(process.env.COURT_SWEEP_INDEX_SRC?.trim());

/** Known eval / legacy org that already holds many criminal docs — READ ONLY. */
const EVAL_ORG = process.env.COURT_SWEEP_EVAL_ORG || "11f3d373-a6d0-4a58-ac72-59b5365dc367";

const MAX_BUNDLE = 220_000;
const DEFAULT_CONCURRENCY = Math.max(1, Number(process.env.COURT_SWEEP_CONCURRENCY || 4));
const DEFAULT_LIMIT = Number(process.env.COURT_SWEEP_LIMIT || 0); // 0 = all remaining

type Route = "BACKEND_LIVE" | "OFFLINE_COURT_PROJECTION" | "SKIP";
...
function writeStatus(partial: {
  ...
  const md = `# COURT CRIMINAL SWEEP — STATUS
...
| Routed OFFLINE_COURT_PROJECTION | ${partial.offline} |
| Routed SKIP | ${partial.skipped} |
| Court scored (ndjson unique keys) | **${partial.scored}** |
| Invent-flag events (sum) | ${partial.inventTotal} |
| Court-fail hitlist rows | ${partial.failHits ?? "—"} |
...
1. Tip SHA \`${PRODUCT_SHA}\` — Court / Chase court-line find-only (no product edits)
...
function loadIndexFromCsv(csvPath: string): IndexRow[] {
  ...
  return rows.map((r) => ({
    ...
    route: (r.route as Route) || "OFFLINE_COURT_PROJECTION",
...
function resolveCriminalUniqueIndex(): IndexRow[] {
  ...
  const src = process.env.COURT_SWEEP_INDEX_SRC?.trim();
  if (src) {
    const abs = path.isAbsolute(src) ? src : path.join(ROOT, src);
    if (!fs.existsSync(abs)) throw new Error(`COURT_SWEEP_INDEX_SRC missing: ${abs}`);
...
> {
  ...
  // Strip modality negation notes — "not full phone download…" is truth prose, not invent claim.
  ...
  if (materialCount > 0) materialFlags.push(`court_claims_${Math.min(materialCount, 40)}`);
...
function projectCourt(bundleTextRaw: string, title: string): {
  ...
  const chase = buildDisclosureChaseBrief({
    caseId: "court-sweep",
    caseTitle: title || "Court sweep case",
    clientLabel,
    allegation,
    stage: "PTPH",
  ...
  if (ledger.hearing?.rawLiteral) claims.push(`HEARING | ${ledger.hearing.rawLiteral}`);
  if (ledger.hearing?.dateIso) claims.push(`HEARING_ISO | ${ledger.hearing.dateIso}`);
  if (chase.hearingDeadlineNote) claims.push(`DEADLINE | ${chase.hearingDeadlineNote}`);
  ...
  for (const item of [...chase.primaryItems, ...chase.additionalItems].slice(0, 24)) {
    ...
    if (item.deadlineLabel) claims.push(`ITEM_DEADLINE | ${item.deadlineLabel}`);
  }

  for (const line of (war as any).sayThis ?? []) {
    if (line) claims.push(`SAY | ${line}`);
  }
  for (const line of (war as any).doNotOverstate ?? []) {
...
/** Emit Court-fail hitlist for fix-queue triage. Volume ≠ guilt. */
  ...
  for (const line of fs.readFileSync(NDJSON, "utf8").split(/\r?\n/)) {
    ...
    rows.push(
      [
        ...
        o.error || "",
        (o.courtClaims || []).slice(0, 3).join(" || ").slice(0, 240),
      ]
...
async function main() {
  ...
  if (indexOnly) {
    writeStatus({
      verdict: "COURT_SWEEP_RUNNING",
      totalUnique: index.length,
      backendLive: index.filter((r) => r.route === "BACKEND_LIVE").length,
      offline: index.filter((r) => r.route === "OFFLINE_COURT_PROJECTION").length,
  ...
  const offlineOnly = process.env.COURT_SWEEP_OFFLINE_ONLY?.trim() === "1";
  if (url && key && !offlineOnly) {
    ...
    console.log("COURT_SWEEP_OFFLINE_ONLY=1 — skipping backend corpus load");
  }
  ...
  for (const row of index) {
    ...
    if (hit) {
      ...
      if (!pdfExists) {
        ...
        row.route = "OFFLINE_COURT_PROJECTION";
        row.skip_reason = "";
      }
    }
  }
  ...
  const routeRank = (r: IndexRow) =>
    r.route === "BACKEND_LIVE" ? 0 : r.route === "OFFLINE_COURT_PROJECTION" ? 1 : 2;
  let pending = index
  ...
  await mapPool(pending, concurrency, async (row) => {
    ...
    try {
      ...
      if (row.route === "BACKEND_LIVE") {
        ...
        if (!bc || bc.chars < 200) {
          ...
          if (row.pdf_path && fs.existsSync(row.pdf_path)) {
            ...
            text = ex.text;
            base.route = "OFFLINE_COURT_PROJECTION";
      ...
      const proj = projectCourt(text, row.display_name || row.source_id || row.case_key);
