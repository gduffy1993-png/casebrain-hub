/**
 * Papers criminal corpus sweep — Chunk P0 find-only (Papers inventory twin of Overview).
 *
 * Projects `buildBundleTruthLedger` materials (Papers inventory surface) vs PDF/source text.
 * Read-only w.r.t product behaviour / DB (service-role READ only).
 * Routes:
 *   BACKEND_LIVE              — reuse extracted_text already on eval/QA cases
 *   OFFLINE_PAPERS_PROJECTION — extract PDF text + Papers ledger claim projectors
 *   SKIP                      — no text, corrupt, non-criminal, duplicate hash
 *
 * Resume:
 *   npx tsx scripts/assurance/papers-criminal-sweep/run-papers-criminal-sweep.ts
 *   npx tsx scripts/assurance/papers-criminal-sweep/run-papers-criminal-sweep.ts --limit=50
 *   npx tsx scripts/assurance/papers-criminal-sweep/run-papers-criminal-sweep.ts --concurrency=6
 *
 * Reuse Overview 2600 index:
 *   PAPERS_SWEEP_REUSE_INDEX=1 PAPERS_SWEEP_INDEX_SRC=artifacts/.../overview-criminal-sweep-v1/CRIMINAL-UNIQUE-INDEX.csv
 */
...
const PRODUCT_SHA = process.env.F167_PRODUCT_SHA?.trim() || "6e63bb6d2";
const PREVIEW =
  process.env.F167_PREVIEW?.replace(/\/$/, "") ||
  "https://casebrain-76gk8vbwk-gduffy1993-pngs-projects.vercel.app";
const DEFAULT_OUT = path.join(ROOT, "artifacts/casebrain-qa/assurance/papers-criminal-sweep-v1");
const OUT_DIR = (() => {
  const raw = process.env.PAPERS_SWEEP_OUT_DIR?.trim();
...
const NDJSON = path.join(OUT_DIR, "papers-sweep.ndjson");
const HITLIST_CSV = path.join(OUT_DIR, "PAPERS-FAIL-HITLIST.csv");
const STATUS_MD = path.join(OUT_DIR, "PAPERS-SWEEP-STATUS.md");
...
const REUSE_INDEX =
  process.env.PAPERS_SWEEP_REUSE_INDEX?.trim() === "1" ||
  Boolean(process.env.PAPERS_SWEEP_INDEX_SRC?.trim());

/** Known eval / legacy org that already holds many criminal docs — READ ONLY. */
const EVAL_ORG = process.env.PAPERS_SWEEP_EVAL_ORG || "11f3d373-a6d0-4a58-ac72-59b5365dc367";

const MAX_BUNDLE = 220_000;
const DEFAULT_CONCURRENCY = Math.max(1, Number(process.env.PAPERS_SWEEP_CONCURRENCY || 4));
const DEFAULT_LIMIT = Number(process.env.PAPERS_SWEEP_LIMIT || 0); // 0 = all remaining

type Route = "BACKEND_LIVE" | "OFFLINE_PAPERS_PROJECTION" | "SKIP";
...
function writeStatus(partial: {
  ...
  const md = `# PAPERS CRIMINAL SWEEP — STATUS
...
| Routed OFFLINE_PAPERS_PROJECTION | ${partial.offline} |
| Routed SKIP | ${partial.skipped} |
| Papers scored (ndjson unique keys) | **${partial.scored}** |
| Invent-flag events (sum) | ${partial.inventTotal} |
| Papers-fail hitlist rows | ${partial.failHits ?? "—"} |
...
1. Tip SHA \`${PRODUCT_SHA}\` — Papers inventory find-only (no product edits)
...
function loadIndexFromCsv(csvPath: string): IndexRow[] {
  ...
  return rows.map((r) => ({
    ...
    route: (r.route as Route) || "OFFLINE_PAPERS_PROJECTION",
...
function resolveCriminalUniqueIndex(): IndexRow[] {
  ...
  const src = process.env.PAPERS_SWEEP_INDEX_SRC?.trim();
  if (src) {
    const abs = path.isAbsolute(src) ? src : path.join(ROOT, src);
    if (!fs.existsSync(abs)) throw new Error(`PAPERS_SWEEP_INDEX_SRC missing: ${abs}`);
...
> {
  ...
  // Papers-specific: inventory collapse (Phase B surface should list materials when source is rich)
  if (materialCount === 0 && richPapers && bundleText.length > 2500) {
    muteFlags.push("mute_papers_inventory_collapse");
  }
  ...
  if (materialCount > 0) materialFlags.push(`papers_materials_${Math.min(materialCount, 40)}`);
...
function projectPapers(bundleTextRaw: string, title: string): {
  claims: string[];
  claimBlob: string;
  ...
  const claims = (ledger.materials ?? [])
    .slice(0, 40)
    .map((m) => {
      const bits = [m.label, m.status, m.detail || "", m.displayLine || "", m.scheduleRef || ""].filter(Boolean);
      return bits.join(" | ");
    })
    .filter(Boolean);

  // Include charge + hearing literals so date-role / invent detectors can see Papers chrome
  if (ledger.charge?.wording) claims.unshift(`CHARGE | ${ledger.charge.wording}`);
  if (ledger.hearing?.rawLiteral) claims.unshift(`HEARING | ${ledger.hearing.rawLiteral}`);
  if (ledger.hearing?.dateIso) claims.unshift(`HEARING_ISO | ${ledger.hearing.dateIso}`);
...
/** Emit Papers-fail hitlist for fix-queue triage. Volume ≠ guilt. */
function writeHitlist(): number {
  ...
  for (const line of fs.readFileSync(NDJSON, "utf8").split(/\r?\n/)) {
    ...
    n++;
    const family =
      invent[0] || modality[0] || mute[0] || dateRole[0] || contra[0] || (o.ok === false ? "engine_error" : "papers_fail");
    rows.push(
      [
        o.unique_key,
        o.case_key,
        ...
        o.error || "",
        (o.papersClaims || []).slice(0, 3).join(" || ").slice(0, 240),
      ]
...
async function main() {
  ...
  if (indexOnly) {
    writeStatus({
      verdict: "PAPERS_SWEEP_RUNNING",
      totalUnique: index.length,
      backendLive: index.filter((r) => r.route === "BACKEND_LIVE").length,
      offline: index.filter((r) => r.route === "OFFLINE_PAPERS_PROJECTION").length,
  ...
  const offlineOnly = process.env.PAPERS_SWEEP_OFFLINE_ONLY?.trim() === "1";
  if (url && key && !offlineOnly) {
    ...
    console.log("PAPERS_SWEEP_OFFLINE_ONLY=1 — skipping backend corpus load");
  }
  ...
  for (const row of index) {
    ...
    if (hit) {
      ...
      if (!pdfExists) {
        ...
        row.route = "OFFLINE_PAPERS_PROJECTION";
        row.skip_reason = "";
      }
    }
  }
  ...
  const routeRank = (r: IndexRow) =>
    r.route === "BACKEND_LIVE" ? 0 : r.route === "OFFLINE_PAPERS_PROJECTION" ? 1 : 2;
  let pending = index
  ...
  await mapPool(pending, concurrency, async (row) => {
    ...
    try {
      ...
      if (row.route === "BACKEND_LIVE") {
        ...
        if (!bc || bc.chars < 200) {
          ...
          if (row.pdf_path && fs.existsSync(row.pdf_path)) {
            ...
            text = ex.text;
            base.route = "OFFLINE_PAPERS_PROJECTION";
      ...
      const proj = projectPapers(text, row.display_name || row.source_id || row.case_key);
      base.engineMs = proj.engineMs;
      base.papersClaims = proj.claims;
