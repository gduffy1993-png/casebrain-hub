/**
 * V2.1.4.4 ordinary-exit system language removal + honest denominator — same frozen 20.
 * Preserves V2.1.4.3 artefacts byte-for-byte. No PDF regen / no scale / no 552 rewrite.
 */
...
const V2144 = path.join(
  ROOT,
  "artifacts/casebrain-qa/assurance/master-auditor-v2/stage3000-diverse-second-v2.1.4.4",
);
...
const V2144_GRAPHS = path.join(
  ROOT,
  "artifacts/casebrain-qa/integrity-programme/diverse3000-v2.1.4.4-pilot-graphs",
);
...
async function main(): Promise<void> {
  fs.mkdirSync(V2144, { recursive: true });
  fs.mkdirSync(path.join(V2144, "ledgers"), { recursive: true });
  fs.mkdirSync(path.join(V2144, "receipts"), { recursive: true });

  writeJson(path.join(V2144, "v2.1.4.3-acceptance-and-scale-pending-note.json"), {
    v2143PreservedByteForByte: true,
    correctedStatus:
      "PILOT_GATE_INCOMPLETE_PENDING_ORDINARY_VISIBLE_SYSTEM_LANGUAGE_REMOVAL__HONEST_STRING_DENOMINATOR",
    sidecar: "../stage3000-diverse-second-v2.1.4.3/pilot-gate-claim-correction.json",
  });
  ...
  fs.copyFileSync(dispSrc, path.join(V2144, "ledgers/v2.1.3-552-occurrence-disposition-ledger.jsonl"));
  ...
  writeJson(path.join(V2144, "remaining-10-fail-closed-reconciliation.json"), {
    schemaVersion: "diverse3000-v2.1.4.4-remaining-10@1.0.0",
    ...
    rows: remain10.map((l) => ({
      ...
      ordinaryExit: false,
      reason:
        "Inventory evidenceStates evidenceAnchor for MG06 referral of absent items — protected audit / inventory, not ordinary solicitor-facing wording. Classifier marks matching anchors as protected_audit_only.",
      remediationInV2144: "protected_audit_only when evidenceAnchor is MG06 index referral / absent-item inventory text",
    })),
    allProtectedOrUnavailable: remain10.every((l) => String(l.jsonPointer || "").includes("/evidenceStates/")),
  });

  const visibleLedger: any[] = [];
  const denominatorBuckets = {
    ordinaryNonEmpty: 0,
    protectedAudit: 0,
    unavailableNotExercised: 0,
    emptyOptional: 0,
    machineMetadata: 0,
  };
  const boundaryScanRows: any[] = [];
  const semanticQualityRows: any[] = [];
  const uniqueOrdinaryTexts = new Map<string, { samplePointer: string; caseId: string }>();
  ...
  for (const row of membership.membership) {
    ...
    for (const leaf of leaves as any[]) {
      ...
      if (isOrdinaryExitLeaf(leaf, classified.classification)) {
        ...
        const isEmptyOrNotExercised =
          !trimmed || classified.classification === "not_exercised";

        // Empty / not-exercised cannot enter the professional-visible denominator or receive professional_pass.
        if (!isEmptyOrNotExercised) {
          const boundary = scanOrdinarySystemLanguageBoundary(trimmed);
          const scan = scanProfessionalSemanticQuality(trimmed);
          const defects = Array.from(new Set([...boundary.defects, ...scan.defects]));
          const disposition = defects.length === 0 ? "professional_pass" : "professional_defect";
          if (boundary.defects.length) systemHarnessHits += 1;
          boundaryScanRows.push({
            caseId,
            jsonPointer: ptr,
            ok: boundary.ok,
            defects: boundary.defects,
          });
          if (!uniqueOrdinaryTexts.has(trimmed)) {
            uniqueOrdinaryTexts.set(trimmed, { samplePointer: ptr, caseId });
          }
          visibleLedger.push({
            caseId,
            surface,
            ...
            detectedReasons: defects,
            proposedSharedCorrection: defects.length === 0
              ? null
              : "Shared-root ordinary-exit system-language removal / professional wording handler",
          });
          if (defects.length) {
            ...
            if (
              defects.includes("markdown_heading_as_charge") ||
              defects.includes("incomplete_charge_as_operative")
            )
              headingAsCharge += 1;
            if (defects.includes("broken_sentence_join")) brokenJoins += 1;
            if (
              defects.includes("raw_absence_identifier") ||
              defects.includes("snake_case_token") ||
              defects.includes("raw_pet_token")
            )
              rawEnums += 1;
            if (defects.includes("lowercase_protected_acronym")) lowerAcronyms += 1;
            if (defects.includes("vague_combined_disclosure")) vagueDisclosure += 1;
            if (defects.includes("ambiguous_section_reference")) unsupportedStatutory += 1;
          }
          ...
          if (/charge/i.test(ptr) || /recorded charge|operative recorded charge/i.test(text)) {
            ...
            const incompleteOperative =
              /operative recorded charge is:/i.test(text) &&
              !isCompleteChargeWording(text.replace(/^.*:\s*/, ""));
            chargeScan.push({
              caseId,
              jsonPointer: ptr,
          ...
          if (/disclosure|chase|limitation|copyLines/i.test(ptr)) {
            disclosureScan.push({
              ...
              defects: defects.filter((d) =>
                /absence|disclosure|abe|referred_absent|vague|system_harness|unit_s|raw_or_duplicated/i.test(
                  d,
                ),
              ),
            });
          }
          if (defects.includes("broken_sentence_join")) {
            sentenceScan.push({ caseId, jsonPointer: ptr, text: text.slice(0, 200) });
          }
          if (
            defects.includes("lowercase_protected_acronym") ||
            defects.includes("raw_pet_token") ||
    ...
    const receiptDir = path.join(V2144, "receipts/cases", caseId);
  ...
  fs.writeFileSync(
    path.join(V2144, "ledgers/professional-visible-string-ledger.jsonl"),
    visibleLedger.map((r) => JSON.stringify(r)).join("\n") + "\n",
    "utf8",
  );
  fs.writeFileSync(
    path.join(V2144, "ledgers/provenance-bound-substantive-leaf-ledger.jsonl"),
  ...
  // Honest denominator accounting over the full provenance-bound leaf ledger
  for (const l of leafLedger) {
    const t = String(l.exactWording || "").trim();
    if (l.classification === "protected_audit_only") denominatorBuckets.protectedAudit += 1;
    else if (l.classification === "machine_metadata") denominatorBuckets.machineMetadata += 1;
    else if (l.classification === "not_exercised") {
      denominatorBuckets.unavailableNotExercised += 1;
    } else if (!t) {
      denominatorBuckets.emptyOptional += 1;
    }
  }
  denominatorBuckets.ordinaryNonEmpty = visibleLedger.length;
  emptyCountedAsPass = visibleLedger.filter(
    (r) =>
      (!String(r.exactWording || "").trim() || r.classification === "not_exercised") &&
      r.professionalQualityDisposition === "professional_pass",
  ).length;

  // Independent semantic-quality pass over every unique non-empty ordinary string
  for (const [text, meta] of uniqueOrdinaryTexts) {
    const scan = scanProfessionalSemanticQuality(text);
    semanticQualityRows.push({
      caseId: meta.caseId,
      samplePointer: meta.samplePointer,
      exactWording: text,
      ok: scan.ok,
      defects: scan.defects,
      reviewerIdentity: null,
      reviewerDecision: null,
      note: "Not qualified solicitor approval — automated semantic-quality pass only",
    });
  }

  writeJson(path.join(V2144, "string-denominator-honesty-report.json"), {
    schemaVersion: "diverse3000-v2.1.4.4-denominator@1.0.0",
    buckets: denominatorBuckets,
    professionalVisibleDenominator: visibleLedger.length,
    emptyOrNotExercisedCountedAsProfessionalPass: emptyCountedAsPass,
    pass: emptyCountedAsPass === 0 && visibleLedger.every((r) => String(r.exactWording || "").trim()),
  });
  writeJson(path.join(V2144, "ordinary-system-language-boundary-scan.json"), {
    schemaVersion: "diverse3000-v2.1.4.4-boundary-scan@1.0.0",
    reviewed: boundaryScanRows.length,
    failCount: boundaryScanRows.filter((r) => !r.ok).length,
    rows: boundaryScanRows.filter((r) => !r.ok).slice(0, 100),
    pass: boundaryScanRows.every((r) => r.ok),
  });
  writeJson(path.join(V2144, "independent-semantic-quality-pass.json"), {
    schemaVersion: "diverse3000-v2.1.4.4-semantic-quality@1.0.0",
    uniqueNonEmptyOrdinaryStrings: semanticQualityRows.length,
    failCount: semanticQualityRows.filter((r) => !r.ok).length,
    reviewerIdentity: null,
    reviewerDecision: null,
    note: "Not qualified solicitor approval",
    rows: semanticQualityRows.filter((r) => !r.ok).slice(0, 100),
    pass: semanticQualityRows.every((r) => r.ok),
  });

  writeJson(path.join(V2144, "professional-semantic-quality-summary.json"), {
    ordinaryVisibleStrings: visibleLedger.length,
    ordinaryNonEmptyDenominator: denominatorBuckets.ordinaryNonEmpty,
    denominatorBuckets,
  ...
  writeJson(path.join(V2144, "charge-completeness-and-heading-scan.json"), {
    rows: chargeScan,
  ...
  writeJson(path.join(V2144, "disclosure-item-wording-scan.json"), {
    rows: disclosureScan.filter((r) => (r.defects || []).length > 0).slice(0, 50),
    pass: disclosureScan.every((r) => (r.defects || []).length === 0),
  });
  writeJson(path.join(V2144, "sentence-composition-scan.json"), {
    brokenJoinCount: sentenceScan.length,
    rows: sentenceScan,
    pass: sentenceScan.length === 0,
  });
  writeJson(path.join(V2144, "acronym-and-procedure-vocabulary-report.json"), {
    rows: acronymScan.slice(0, 80),
    lowercaseAcronymDefects: lowerAcronyms,
    pass: lowerAcronyms === 0,
  });
  writeJson(path.join(V2144, "before-after-wording-map.json"), { rows: wordingMapAll });
  writeJson(path.join(V2144, "derived-conclusion-dependency-ledger.json"), {
    rows: derivedDeps,
    missingDependencyCount: derivedMissing,
    pass: derivedMissing === 0,
  });
  writeJson(path.join(V2144, "all-exit-regression-matrix.json"), {
    rows: exitMatrix,
  ...
  writeJson(path.join(V2144, "before-after-regression-report.json"), {
    beforeV2142: {
  ...
  writeJson(path.join(V2144, "contracts-proof.json"), {
  ...
  try {
    contractsStdout = execFileSync(
      ...
      [
        ...
        "tsx",
        "--test",
        "scripts/assurance/stage3000-diverse-second/v2.1.4.4-focused-contracts.test.ts",
      ],
      {
        cwd: ROOT,
        encoding: "utf8",
  ...
  writeJson(path.join(V2144, "contracts-test-receipt.json"), {
  ...
  try {
    tscStdout = execFileSync(
      process.execPath,
      [tscBin, "-p", "tsconfig.v2144-path-scoped.json", "--noEmit"],
  ...
  const changedRel = [
    "scripts/assurance/stage3000-diverse-second/v2.1.4.4-ordinary-exit-system-language.ts",
    ...
    "tsconfig.v2144-path-scoped.json",
    "artifacts/casebrain-qa/assurance/master-auditor-v2/stage3000-diverse-second-v2.1.4.3/pilot-gate-claim-correction.json",
  ];
  ...
  writeJson(path.join(V2144, "build-and-tsc-receipt.json"), {
    pathScopedTsc: {
      exitCode: tscExit,
      startedAt: tscStartedAt,
      finishedAt: tscFinishedAt,
      command: "node node_modules/typescript/bin/tsc -p tsconfig.v2144-path-scoped.json --noEmit",
  ...
  writeJson(path.join(V2144, "CHANGED-FILE-MANIFEST.json"), {
  ...
  if (systemHarnessHits !== 0) blockers.push("system_harness_language");
  if (emptyCountedAsPass !== 0) blockers.push("empty_counted_as_professional_pass");
  if (visibleLedger.some((r) => !String(r.exactWording || "").trim())) {
    blockers.push("empty_in_professional_denominator");
  }
  ...
  if (
    !semanticProof.markdownHeadingFails ||
    !semanticProof.section45DoesNotOverstate ||
    !semanticProof.unitSFails ||
    !semanticProof.apiExitFails ||
    !semanticProof.protectedAuditFails ||
    !semanticProof.rawDocPointerFails
  ) {
    blockers.push("semantic_contracts");
  }

  const gatePass = blockers.length === 0;
  writeJson(path.join(V2144, "pilot-gate-result.json"), {
    gatePass,
    ...
    v2143ClaimCorrected: "PILOT_GATE_INCOMPLETE_PENDING_ORDINARY_VISIBLE_SYSTEM_LANGUAGE_REMOVAL__HONEST_STRING_DENOMINATOR",
  });

  writeJson(path.join(V2144, "DECISION-CARD.json"), {
    verdict: gatePass
      ? "PILOT_GATE_PASS_STOP_FOR_CODEX_BEFORE_SCALE"
      : "PILOT_GATE_INCOMPLETE_PENDING_ORDINARY_VISIBLE_SYSTEM_LANGUAGE_REMOVAL__HONEST_STRING_DENOMINATOR",
    scaleGateAccepted: false,
  ...
  fs.writeFileSync(
    path.join(V2144, "DECISION-CARD.md"),
    [
      "# V2.1.4.4 Decision Card",
      "",
      `**Verdict:** ${gatePass ? "PILOT_GATE_PASS_STOP_FOR_CODEX_BEFORE_SCALE" : "INCOMPLETE_PENDING_ORDINARY_VISIBLE_SYSTEM_LANGUAGE_REMOVAL__HONEST_STRING_DENOMINATOR"}`,
      `- Scale gate accepted: **false** (do not set true in this stop)`,
      `- Professional semantic defects: ${professionalDefectCount}`,
      `- System/harness hits: ${systemHarnessHits}; empty-as-pass: ${emptyCountedAsPass}`,
      `- Ordinary non-empty denominator: ${denominatorBuckets.ordinaryNonEmpty}`,
  ...
  writeJson(path.join(V2144, "STOP-FOR-CODEX-REVIEW.json"), {
    schemaVersion: "STOP-FOR-CODEX-REVIEW@1.0.0",
    stoppedAt: new Date().toISOString(),
    reason:
      "V2.1.4.4 ordinary-visible system language removal + honest string denominator — stop uncommitted for Codex review before scale",
    gatePass,
    scaleRecommended: false,
    scaleGateAccepted: false,
    v2143PreservedByteForByte: true,
    ...
    deliverables: {
      ...
      denominatorHonesty: "string-denominator-honesty-report.json",
      boundaryScan: "ordinary-system-language-boundary-scan.json",
      semanticQualityPass: "independent-semantic-quality-pass.json",
  ...
  writeJson(path.join(V2144, "occurrence-disposition-summary.json"), {
    total: 552,
