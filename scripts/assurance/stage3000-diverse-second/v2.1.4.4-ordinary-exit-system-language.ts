...
const HANDLER = "v2.1.4.4-ordinary-exit-system-language@1.0.0";
...
export function extractValidatedChargeWording(matter: any, pageText: string): {
  ...
  if (
    ...
    return {
      ...
      reason: isCompleteChargeWording(fromMatter)
        ? null
        : "The extracted text does not confirm the complete charge wording or legal basis.",
    };
  }
  ...
  for (const re of patterns) {
    ...
    if (frag && !isChargeHeadingOrLabel(frag) && frag.length >= 24) {
      return {
        ...
        foundFragment: frag,
        reason:
          "The extracted text contains only incomplete particulars and does not confirm the complete charge wording or legal basis.",
      };
    }
  }
  ...
  return {
    ...
    reason: headingHit
      ? "The extracted text contains only a heading and does not confirm the complete charge wording or legal basis."
...
export function evidenceItemsProfessionalPhrase(count: number): string {
  const n = Math.max(0, Math.floor(Number(count) || 0));
  if (n === 0) {
    return "No evidence items are available on the papers; the pack does not establish the allegation.";
  }
  const label = NUMBER_WORDS[n] || String(n);
  if (n === 1) {
    return "One evidence item is present on the papers; it does not by itself establish the allegation against the defendant.";
  }
  return `${label} evidence items are present on the papers; they do not by themselves establish the allegation against the defendant.`;
}

function isHarnessChargeFragment(text: string): boolean {
  return /not pinned|structural only|charge wording not|statutory or common-law cue/i.test(
    String(text || ""),
  );
}

function professionalInstrumentLabel(instrumentType: string, sourceDocument: string | null): string {
  const raw = String(sourceDocument || instrumentType || "charge instrument")
    .replace(/_/g, " ")
    .trim()
    .toLowerCase();
  if (/written\s*charge|charge\s*sheet|mg\s*4|mg04/.test(raw)) return "written charge";
  if (/indictment/.test(raw)) return "indictment";
  if (/summons/.test(raw)) return "summons";
  if (/information/.test(raw)) return "information";
  return "charge instrument";
}

function professionalSourceLocationLine(args: {
  instrumentType: string;
  sourceDocument: string | null;
  pageNo: string | null;
  complete: boolean;
}): string {
  const label = professionalInstrumentLabel(args.instrumentType, args.sourceDocument);
  const page = args.pageNo && /^\d+$/.test(args.pageNo) ? args.pageNo : null;
  if (args.complete && page) {
    if (label === "written charge") return `The operative written charge appears at page ${page}.`;
    if (label === "indictment") return `The operative indictment appears at page ${page}.`;
    return `The operative ${label} appears at page ${page}.`;
  }
  if (page) {
    return `An instrument is present at page ${page}, but complete charge wording is not confirmed. Check the operative charge sheet or amendment record.`;
  }
  return "No charge instrument page is available in the served pack; the charge position remains unresolved.";
}

export function mg6ReferralProvenancePhrase(pageNo: string | null): string {
  const page = pageNo && /^\d+$/.test(pageNo) ? pageNo : "1";
  return `The MG6 schedule refers to this material at page ${page}. The material itself has not been supplied, so its contents cannot be summarised.`;
}
...
export function defenceProse(raw: string): {
  ...
  if (/section\s*45|s\.?\s*45|indicator/.test(d)) {
    return {
      text: "The papers refer to a possible section 45 issue. The statutory basis and factual foundation are not confirmed on the current pack, so no statutory defence is identified.",
      unresolved: true,
      limitation: "Section 45 reference lacks confirmed legislation and factual basis.",
...
export function chargeStatusProse(args: {
  ...
  if (!chargeExtract.complete) {
    const fragRaw = chargeExtract.foundFragment
      ...
      : null;
    const quoteHarness = fragRaw && isHarnessChargeFragment(fragRaw);
    const text =
      fragRaw && !quoteHarness
        ? "The extracted text contains only a heading and does not confirm the complete charge wording or legal basis. Check the operative charge sheet or amendment record."
        : "The extracted text contains only a heading and does not confirm the complete charge wording or legal basis. Check the operative charge sheet or amendment record.";
    return {
      text,
      unresolved: true,
...
export type SemanticDefect =
  ...
  | "system_harness_language"
  | "unit_s_pluralisation"
  | "raw_or_duplicated_document_pointer";

/** Deterministic boundary scan — system/harness language banned from ordinary exits. */
export function scanOrdinarySystemLanguageBoundary(text: string): {
  ok: boolean;
  defects: SemanticDefect[];
} {
  const defects: SemanticDefect[] = [];
  const s = String(text || "");
  // Empty strings are excluded from the professional-visible denominator; do not scan as ordinary prose.
  if (!s.trim()) {
    return { ok: true, defects: [] };
  }
  if (/unit\(s\)/i.test(s)) defects.push("unit_s_pluralisation");
  if (/page identity/i.test(s)) defects.push("system_harness_language");
  if (/API exit/i.test(s)) defects.push("system_harness_language");
  if (/browser remains not exercised/i.test(s)) defects.push("system_harness_language");
  if (/protected audit/i.test(s)) defects.push("system_harness_language");
  if (/\banchors to\b/i.test(s)) defects.push("system_harness_language");
  if (/without inventing/i.test(s)) defects.push("system_harness_language");
  if (/not pinned/i.test(s)) defects.push("system_harness_language");
  if (/structural only/i.test(s)) defects.push("system_harness_language");
  if (/\bnot_exercised\b/i.test(s)) defects.push("system_harness_language");
  if (/\b(handlers?|schemas?|audit metadata)\b/i.test(s)) defects.push("internal_process_language");
  if (/without page content/i.test(s)) defects.push("system_harness_language");
  if (/statutory or common-law cue/i.test(s)) defects.push("system_harness_language");
  if (/inventing master content|invented content/i.test(s)) defects.push("system_harness_language");
  // Raw / duplicated document pointer forms e.g. "MG6 MG06/page/1"
  if (/\bMG\s*\d+\s+MG\d{2}\/page\/\d+/i.test(s) || /\bMG\d{2}\/page\/\d+/i.test(s)) {
    defects.push("raw_or_duplicated_document_pointer");
  }
  if (/\/page\/\d+/i.test(s)) defects.push("raw_or_duplicated_document_pointer");
  if (/\b[a-f0-9]{32,64}\b/i.test(s)) defects.push("system_harness_language");
...
export function scanProfessionalSemanticQuality(text: string): {
  ...
  const defects: SemanticDefect[] = [];
  const s = String(text || "");
  const boundary = scanOrdinarySystemLanguageBoundary(s);
  for (const d of boundary.defects) {
    if (!defects.includes(d)) defects.push(d);
  }
...
export function buildProfessionalSolicitorSurfaces(args: WordingBuildArgs): BuiltSolicitorSurfaces {
  ...
  const instrumentType = args.chargeInstrument
    ...
    : "charge instrument";
  const pageNoFromRef =
    chargeRef?.sourcePage?.match(/\/page\/(\d+)/)?.[1] ||
    (chargeRef?.sourcePage && /^\d+$/.test(chargeRef.sourcePage) ? chargeRef.sourcePage : null) ||
    (chargeRef?.sourcePage?.match(/page\s+(\d+)/i)?.[1] ?? null);
  const chargesSource = args.chargeInstrument
    ? professionalSourceLocationLine({
        instrumentType,
        sourceDocument: args.chargeInstrument.sourceDocument
          ? String(args.chargeInstrument.sourceDocument)
          : null,
        pageNo: pageNoFromRef,
        complete: args.chargeExtract.complete,
      })
  ...
  const missingNote =
    distinctItems.length > 0
      ? `The following referred material has not been supplied and must not be treated as served: ${distinctItems.join("; ")}.`
  ...
  const whatEvidenceSays = evidenceItemsProfessionalPhrase(args.servedEvidenceCount);
  put(
  ...
  const mg06PageNo =
    mg06Ref?.sourcePage?.match(/\/page\/(\d+)/)?.[1] ||
    (mg06Ref?.sourcePage && /^\d+$/.test(mg06Ref.sourcePage) ? mg06Ref.sourcePage : null) ||
    args.mg06?.sourcePageNo ||
    "1";
  const provenanceFive = mg06Ref
    ? mg6ReferralProvenancePhrase(mg06PageNo)
    : "No MG6 schedule page is available for the referred material that has not been supplied; its contents cannot be summarised.";
  put(
  ...
  const surfaces = {
    ...
    controlRoom: {
      ...
      whyItMatters:
        "Missing or partial disclosure changes what may safely be said in any copied or exported summary.",
      safePosition:
        "Solicitor review is required before send. Any copied or exported summary must retain this limitation until the missing material is supplied.",
      nextAction: chaseFinal
        ? "Chase only the particularised missing item."
        : "No specific outstanding disclosure item has been identified from the available papers.",
    },
    disclosureChase: {
      item: disclosureItem,
...
export function proveProfessionalSemanticContracts(): {
  ...
  unitSFails: boolean;
  pageIdentityFails: boolean;
  apiExitFails: boolean;
  browserNotExercisedFails: boolean;
  protectedAuditFails: boolean;
  anchorsToFails: boolean;
  withoutInventingFails: boolean;
  notPinnedFails: boolean;
  structuralOnlyFails: boolean;
  rawDocPointerFails: boolean;
  detail: string;
} {
  ...
  const unitS = scanOrdinarySystemLanguageBoundary("3 evidence unit(s) with page identity are present.");
  const pageId = scanOrdinarySystemLanguageBoundary("evidence items with page identity are present.");
  const api = scanOrdinarySystemLanguageBoundary("safely be said on copy, export, and API exits.");
  const browser = scanOrdinarySystemLanguageBoundary(
    "Solicitor review is required before send. Authenticated browser remains not exercised.",
  );
  const prot = scanOrdinarySystemLanguageBoundary("Keep identifiers in protected audit records only.");
  const anchors = scanOrdinarySystemLanguageBoundary(
    "Provenance anchors to MG6 MG06/page/1 without inventing master content.",
  );
  const invent = scanOrdinarySystemLanguageBoundary("without inventing master content");
  const pinned = scanOrdinarySystemLanguageBoundary("Charge wording not pinned — structural only.");
  const structural = scanOrdinarySystemLanguageBoundary("structural only extract");
  const rawPtr = scanOrdinarySystemLanguageBoundary("anchors to MG6 MG06/page/1");
  return {
    ...
    section45DoesNotOverstate:
      s45map.unresolved &&
      /possible section 45/i.test(s45map.text) &&
      !/not pinned/i.test(s45map.text),
    unitSFails: !unitS.ok,
    pageIdentityFails: !pageId.ok,
    apiExitFails: !api.ok,
    browserNotExercisedFails: !browser.ok,
    protectedAuditFails: !prot.ok,
    anchorsToFails: !anchors.ok,
    withoutInventingFails: !invent.ok,
    notPinnedFails: !pinned.ok,
    structuralOnlyFails: !structural.ok,
    rawDocPointerFails: !rawPtr.ok,
