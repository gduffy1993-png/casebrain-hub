#!/usr/bin/env npx tsx
/**
 * Messy PDF-backed proof pack + proof receipts (v4 scale-300).
 *
 * Run:
 *   npx tsx scripts/run-messy-pdf-proof-v4-scale300.ts
 */
...
const OUT_ROOT = path.join(ROOT, "artifacts", "casebrain-qa", "messy-pdf-proof-v4-scale300");
...
function buildScenarios(): ScenarioSpec[] {
  ...
  if (scenarios.length !== 300) {
    throw new Error(`Expected 300 scenarios but built ${scenarios.length}`);
  }
  return scenarios;
}
...
function writeRepeatedPatterns(runs: CaseRun[]) {
  ...
  const md = [
    "# REPEATED PATTERNS — messy-pdf-proof-v4-scale300",
    "",
    "| Pattern | Hits |",
    "|---------|-----:|",
...
function writeSafeToScaleVerdict(runs: CaseRun[]) {
  ...
  const md = [
    "# SAFE-TO-SCALE-VERDICT — messy-pdf-proof-v4-scale300",
    "",
    "## Hard-safety gate",
    "",
    `- 300/300 build: **${totals.casesRun === 300 ? "yes" : "no"}**`,
