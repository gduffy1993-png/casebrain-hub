...
async function contractExactPageProvenance() {
  ...
  await check("printed pagination is read only from the header/footer band", () => {
    ...
    assert.deepEqual(readPrintedSourcePagination("CB-TB-343    Page 8Prepared papers\nMG6 row."), {
      sourcePage: 8,
  ...
  await check("old whole-document text with printed page headers recovers exact page units", () => {
    const oldUploadText = [
      "CB-TB-343    Page 1Prepared papers\nPROSECUTION PAPERS\nR v Ellis Dunn",
      "CB-TB-343    Page 2Prepared papers\nCHARGE SHEET\nConspiracy to burgle",
      [
        "CB-TB-343    Page 3Prepared papers",
        "MG6 DISCLOSURE SCHEDULE",
        "O01full interview transcriptOutstandingListed but not attached",
        "O02CAD log full printOutstandingNot yet served",
      ].join("\n"),
    ].join("\n");
    const units = pageUnitsFromExtractedText(oldUploadText);
    assert.ok(units, "printed page headers should split old upload text");
    assert.equal(units!.length, 3);
    assert.deepEqual(units!.map((u) => u.compiledPage), [1, 2, 3]);
    assert.deepEqual(units!.map((u) => u.sourcePage), [1, 2, 3]);
