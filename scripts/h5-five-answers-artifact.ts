#!/usr/bin/env npx tsx
/**
 * H5 Five Answers — Layer 7 artifact from golden pack sample.
 * Run: npx tsx scripts/h5-five-answers-artifact.ts
 */
...
const OUT_DIR = path.join(process.cwd(), "artifacts", "casebrain-qa", "h5-five-answers");
...
function main(): void {
  ...
  const report = {
    ...
    level: "H5 Five Answers — Layer 7 sample",
    samples,
  };

  fs.mkdirSync(OUT_DIR, { recursive: true });
  fs.writeFileSync(path.join(OUT_DIR, "five-answers-artifact.json"), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`Wrote ${path.join(OUT_DIR, "five-answers-artifact.json")} (${samples.length} samples)`);
}
