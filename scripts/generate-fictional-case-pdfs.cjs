/**
 * Generate selectable-text PDFs from docs/fictional-cases-40/NS-CPS-*.txt
 * Usage: npm run generate:fictional-pdfs
 */
const fs = require("fs");
...
async function main() {
  ...
  for (const f of files) {
    const text = fs.readFileSync(path.join(DOCS_DIR, f), "utf-8");
