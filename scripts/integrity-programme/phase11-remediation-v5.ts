 ...
 *
 * Run: npx tsx scripts/integrity-programme/phase11-remediation-v5.ts
 */
...
const V5 = path.join(OUT, "v5");
const RENDER = path.join(V5, "rendered");
const BUNDLE = path.join(V5, "reviewer-bundle");
const BEFORE_AFTER = path.join(OUT, "before-after-surfaces-v5");
...
function writeBlindMarkdown(goldId: string, surfaces: Surface[], eligibility: string) {
  const lines = [
    `# ${goldId} — solicitor-visible render (v5)`,
    "",
...
function renderSynthetic(def: SampleDef): RenderResult {
  ...
  if (id === "SYN-TRUNC-01") {
    ...
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: hay,
      auditFamily,
      surfaceId: "phase11_v5_trunc_copy",
      mode: "copy",
    });
    surfaces.push({
    ...
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: hay,
      auditFamily,
      surfaceId: "phase11_v5_placeholder",
      mode: "copy",
    });
    surfaces.push({
      surface: "copy_preview",
    ...
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: hay,
      auditFamily,
      surfaceId: "phase11_v5_family_export",
      mode: "export",
    });
    surfaces.push({
    ...
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: hay,
      auditFamily,
      surfaceId: "phase11_v5_safe_copy",
      mode: "copy",
    });
    surfaces.push({
      surface: "copy_preview",
...
function renderGoldManual(def: SampleDef): RenderResult {
  ...
  if (summary.courtLine) {
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: bundleHayForFamily,
      auditFamily,
      surfaceId: "phase11_v5_court_line",
      mode: "copy",
    });
    surfaces.push({
      surface: "court_line",
  ...
  // v5 blocking-FN fix: client_summary must never set canCopy=true without the full
  // sanitize → boundary → assert → gate → gated-copy discipline. GOLD-11-039's
  // client_summary is the case that exposed the v4 miss (see header comment).
  if (summary.clientSummaryPreview) {
    const vis = renderCopyableSolicitorText({
      rawText: summary.clientSummaryPreview,
      allegation: summary.allegation ?? "",
      bundleHay: bundleHayForFamily,
      auditFamily,
      surfaceId: "phase11_v5_client_summary",
      mode: "copy",
  ...
  for (const label of chaseLabels.slice(0, 4)) {
    ...
    const rawDraft = (chase?.draft ?? label).replace(/\bmg(\d+[a-z]?)\b/gi, (_, n: string) => `MG${String(n).toUpperCase()}`);
    const vis = renderCopyableSolicitorText({
      rawText: rawDraft,
      allegation: summary.allegation ?? "",
      bundleHay: bundleHayForFamily,
      auditFamily,
      surfaceId: "phase11_v5_cps_chase",
      mode: "copy",
...
function renderEsa(def: SampleDef): RenderResult {
  ...
  sampleTexts.forEach((t, idx) => {
    const vis = renderCopyableSolicitorText({
      ...
      bundleHay: hay,
      auditFamily,
      surfaceId: "phase11_v5_case_copy",
      mode: "copy",
      itemIndex: idx,
    });
    surfaces.push({
  ...
  const leakVis = renderCopyableSolicitorText({
    ...
    bundleHay: hay,
    auditFamily,
    surfaceId: "phase11_v5_leak_probe",
    mode: "copy",
  });
  surfaces.push({
...
function main() {
  ...
  // Preserve v1 + v2 + v3 + v4 — never write into V1, PREV_V2, PREV_V3, or PREV_V4 paths.

  // Membership: carry forward the SAME 43 v4 goldIds/fixtures, UNCHANGED (lineage).
  // No additions, no removals — v5 is identical membership to v4, only render/scan differ.
  ...
  // Lineage integrity assertion: the reconstructed v5 membership (v1 33 + the v4
  // gold-manual additions) must be EXACTLY the same goldId/fixtureId set as v4's frozen
  // membership — no drift permitted between v4 and v5.
  const v4MembershipKey = new Set(v4Frozen.samples.map((s) => `${s.goldId}|${s.fixtureId}`));
  const v5MembershipKey = new Set(samples.map((s) => `${s.goldId}|${s.fixtureId}`));
  if (v4MembershipKey.size !== v5MembershipKey.size || [...v4MembershipKey].some((k) => !v5MembershipKey.has(k))) {
    throw new Error("v5 membership drifted from v4 frozen membership — lineage must be identical (0 additions, 0 removals)");
  }

  if (samples.length < MEMBERSHIP_MIN || samples.length > MEMBERSHIP_MAX) {
    throw new Error(`v5 membership ${samples.length} outside target range ${MEMBERSHIP_MIN}-${MEMBERSHIP_MAX}`);
  }
  ...
  fs.writeFileSync(path.join(V5, "gold-sample-frozen-v5.json"), JSON.stringify(frozenSample, null, 2), "utf8");
  ...
  for (const def of samples) {
    ...
    }>(path.join(PREV_V4, "rendered", `${def.goldId}.json`));
  ...
  fs.writeFileSync(
    path.join(OUT, "automated-predictions-v5.json"),
    JSON.stringify(
      {
  ...
  const keyComparisons = KEY_COMPARISON_GOLDS.map((goldId) => {
    ...
    const v5 = readJson<{ surfaces: Surface[]; reviewEligibility?: string }>(path.join(RENDER, `${goldId}.json`));
    ...
    return {
      goldId,
      reviewEligibility: v5?.reviewEligibility ?? null,
      surfaceCounts: {
        v1: v1Fp.length,
        v2: v2Fp.length,
        v3: v3Fp.length,
  ...
  fs.writeFileSync(
    path.join(OUT, "automated-comparison-report-v5.json"),
    JSON.stringify(
      {
        generatedAt: frozenAt,
        parentV1FreezeHash: V1_FREEZE_HASH,
  ...
  fs.writeFileSync(path.join(V5, "human-judgment-workbook.json"), JSON.stringify(humanWorkbook, null, 2), "utf8");
...
**Parent v4 freeze hash (preserved):** \`${V4_FREEZE_HASH}\`
...
`,
  ...
  const contracts = [
    ...
    { name: "v5_frozen_total", pass: v5Defs.length === samples.length, detail: String(v5Defs.length) },
    {
      name: "membership_identical_to_v4",
      pass: v4MembershipKey.size === v5MembershipKey.size && [...v4MembershipKey].every((k) => v5MembershipKey.has(k)),
  ...
  const report = {
    ...
    status: "REMEDIATION_V5_COMPLETE__AWAITING_HUMAN_GOLD_REVIEW",
    generatedAt: frozenAt,
    parentV1FreezeHash: V1_FREEZE_HASH,
    parentV2FreezeHash: V2_FREEZE_HASH,
    parentV3FreezeHash: V3_FREEZE_HASH,
  ...
  fs.writeFileSync(path.join(OUT, "phase11-remediation-v5-report.json"), JSON.stringify(report, null, 2), "utf8");
  fs.writeFileSync(
    path.join(OUT, "before-after-summary-v5.json"),
  ...
  const dispositionReport = {
    ...
    sites: [
      ...
      {
        site: "scripts/integrity-programme/phase11-remediation-v5.ts (THIS script) — client_summary / court_line / cps_chase_draft / copy_preview / export_preview / family_leak_probe",
        priorBehavior:
  ...
  fs.writeFileSync(
    path.join(OUT, "fixed-length-wording-operations-v5.json"),
    JSON.stringify(dispositionReport, null, 2),
    "utf8",
  );

  const checkpoint = `# Phase 11 remediation v5 checkpoint
...
**v5 membership policy:** ${lineageDisclosure.v5MembershipPolicy}
...
`;
  ...
  fs.writeFileSync(path.join(OUT, "PHASE-11-REMEDIATION-V5-CHECKPOINT.md"), checkpoint, "utf8");
  ...
  if (fs.existsSync(readmePath)) {
    ...
    if (readme.includes("Phase 11 —")) {
      readme = readme.replace(
        /\| Phase 11 —.*\|/,
        "| Phase 11 — rendered coverage + gold FP–FN | **REMEDIATION_V5_COMPLETE / AWAITING_HUMAN_GOLD_REVIEW** (v1+v2+v3+v4 preserved; v5 membership identical to v4, blocking FN fixed, not a PASS) | `docs/integrity-programme/phase-11-remediation-v5-checkpoint.md` |",
      );
