/**
 * Phase 11 remediation — regenerate blinded v3 comparison set from frozen v1 membership.
 * Preserves v1 and prior v2 artefacts as before/after evidence. Does NOT modify them.
 *
 * Run: npx tsx scripts/integrity-programme/phase11-remediation-v3.ts
 */
...
const V3 = path.join(OUT, "v3");
const RENDER = path.join(V3, "rendered");
const BUNDLE = path.join(V3, "reviewer-bundle");
const BEFORE_AFTER = path.join(OUT, "before-after-surfaces-v3");
...
function writeBlindMarkdown(goldId: string, surfaces: Surface[], eligibility: string) {
  const lines = [
    `# ${goldId} — solicitor-visible render (v3)`,
    "",
...
function renderSynthetic(def: SampleDef): RenderResult {
  ...
  const ctx = buildRedactedSourceContext({
    available: false,
    reasonIfMissing:
      "Controlled synthetic integrity probe — no controlled client bundle or truth-key is attached to this gold id.",
    allegation,
  });
  surfaces.push({
    ...
    sourceEvidenceNote: "synthetic — insufficient controlled source",
  ...
  if (id === "SYN-TRUNC-01") {
    ...
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_trunc_copy",
      texts: [text],
      allegation,
      bundleHay: hay,
      mode: "copy",
    ...
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_placeholder",
      texts: [text],
      allegation,
      bundleHay: hay,
      mode: "copy",
    ...
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_family_export",
      texts: [text],
      allegation,
      bundleHay: hay,
    ...
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_safe_copy",
      texts: [text],
      allegation,
      bundleHay: hay,
      auditFamily: "harassment_digital",
...
function renderGoldManual(def: SampleDef): RenderResult {
  ...
  if (summary.courtLine) {
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_court_line",
  ...
  const chaseLabels = dedupeSolicitorLabels((summary.cpsChase ?? []).map((c) => c.label));
...
function renderEsa(def: SampleDef): RenderResult {
  ...
  const chaseLabels = dedupeSolicitorLabels(chaseBrief.items.map((it) => it.label)).slice(0, 8);
  surfaces.push({
    surface: "chase_brief",
  ...
  sampleTexts.forEach((t, idx) => {
    const gated = gateSolicitorOutput({
      surfaceId: "phase11_v3_case_copy",
      texts: [t],
      allegation,
      bundleHay: hay,
      auditFamily,
  ...
  const leakGate = gateSolicitorOutput({
    surfaceId: "phase11_v3_leak_probe",
    texts: [leakProbe],
    allegation,
    bundleHay: hay,
    auditFamily,
...
function main() {
  ...
  // Preserve v1 + v2 — never write into V1 or PREV_V2 paths.
  ...
  const duplicationDisclosure = {
    ...
    v3MembershipPolicy:
      "SAME 33 goldIds / fixtures regenerated for comparison. Exact fixture repeats across strata are JUSTIFIED (multi-angle review of the same papers: accepted vs blocked vs uncertain). Membership not silently changed. v1 and v2 artefacts preserved read-only.",
  };

  const frozenAt = new Date().toISOString();
  const v3Defs = samples.map((s) => ({
    goldId: s.goldId,
  ...
  fs.writeFileSync(path.join(V3, "gold-sample-frozen-v3.json"), JSON.stringify(frozenSample, null, 2), "utf8");
  ...
  for (const def of samples) {
    ...
    const v2Json = readJson<{
      surfaces: Array<{ surface: string; solicitorVisibleText: string; gateStatus: string; canCopy: boolean | null }>;
    }>(path.join(PREV_V2, "rendered", `${def.goldId}.json`));
    const changes: unknown[] = [];
    const v2Surfaces = v2Json?.surfaces ?? [];
    for (let i = 0; i < Math.max(surfaces.length, v2Surfaces.length); i++) {
      const after = surfaces[i];
      const before = v2Surfaces[i];
      if (!after && before) {
        changes.push({
    ...
    predictions.push({
      ...
      evidenceReference: `phase-11-remediation/v3/rendered/${def.goldId}.md`,
    });
    humanSlots.push({
      goldId: def.goldId,
      reviewEligibility: eligibility,
      ...
      rationale: null,
      evidenceReference: `artifacts/casebrain-qa/integrity-programme/phase-11-remediation/v3/rendered/${def.goldId}.md`,
  ...
  fs.writeFileSync(
    path.join(OUT, "automated-predictions-v3.json"),
    JSON.stringify(
      {
  ...
  const keyComparisons = KEY_COMPARISON_GOLDS.map((goldId) => {
    ...
    const v3 = readJson<{ surfaces: Surface[]; reviewEligibility?: string }>(
      path.join(RENDER, `${goldId}.json`),
    );
    ...
    return {
      goldId,
      reviewEligibility: v3?.reviewEligibility ?? null,
      surfaceCounts: {
        v1: v1Fp.length,
        v2: v2Fp.length,
        v3: v3Fp.length,
      ...
      v3Preview: v3Fp.slice(0, 4),
    };
  });

  fs.writeFileSync(
    path.join(OUT, "automated-comparison-report-v3.json"),
    JSON.stringify(
      {
        generatedAt: frozenAt,
        parentV1FreezeHash: V1_FREEZE_HASH,
        ...
        summary: {
          ...
          changedV2toV3Count: keyComparisons.filter((c) => c.changedV2toV3).length,
  ...
  const humanWorkbook = {
    ...
    instructions: [
      "v3 remediation comparison set — awaiting independent qualified human review.",
  ...
  fs.writeFileSync(path.join(V3, "human-judgment-workbook.json"), JSON.stringify(humanWorkbook, null, 2), "utf8");
  ...
  const blindOrder = {
    purpose:
      "Blinded v3 order — no automated predictions, stratum, selectionReason, reviewFocus, or expected classification",
    freezeHash,
    parentV1FreezeHash: V1_FREEZE_HASH,
    parentV2FreezeHash: V2_FREEZE_HASH,
  ...
  for (const s of samples) {
    ...
    const packet = {
      ...
      sourceContextNote:
        "See Controlled source context surface; INSUFFICIENT_SOURCE_CONTEXT cases are excluded from substantive FP/FN.",
    };
  ...
  fs.writeFileSync(
    ...
    `# Phase 11 v3 gold review — instructions

**Status:** AWAITING_HUMAN_GOLD_REVIEW (remediation comparison set)  
**v3 freeze hash:** \`${freezeHash}\`  
...
Follow \`blinded-review-order.json\`. Do not consult automated-predictions-v3.json while judging.
`,
    "utf8",
  );
  ...
  fs.writeFileSync(
    ...
    `# Phase 11 v3 — human judgment form
...
`,
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        bundleId: "phase11-reviewer-bundle-v3",
        freezeHash,
        parentV1FreezeHash: V1_FREEZE_HASH,
        parentV2FreezeHash: V2_FREEZE_HASH,
  ...
  const fnChecks = {
    ...
    "GOLD-11-029_api_neutral_banner_only":
      g029?.surfaces.some((s) => s.solicitorVisibleText === NEUTRAL_SOLICITOR_BLOCKED_BANNER) === true &&
      g029?.surfaces.every((s) => !/NOT USABLE|consumer|debug/i.test(s.solicitorVisibleText)) === true,
  };
  ...
  const contracts = [
    ...
    { name: "v3_frozen_33", pass: v3Defs.length === 33, detail: String(v3Defs.length) },
  ...
  const report = {
    phase: "11-remediation-v3",
    status: "REMEDIATION_V3_COMPLETE__AWAITING_HUMAN_GOLD_REVIEW",
    generatedAt: frozenAt,
    parentV1FreezeHash: V1_FREEZE_HASH,
    parentV2FreezeHash: V2_FREEZE_HASH,
    v3FreezeHash: freezeHash,
    ...
    rootCausesFixed: [
      "Controlled source context surface with INSUFFICIENT_SOURCE_CONTEXT exclusion from substantive FP/FN",
      ...
      "Client summary prose sanitized (redacted papers / unsafe proof-outcome wording)",
      "Evidence states humanized (referred_only → Referred only)",
      "Chase labels deduped for solicitor display",
      "Blocked copy previews proportionate (Item/Status/Reason) via ruleIds",
      "SYN-API-BLOCK-01 shows only NEUTRAL_SOLICITOR_BLOCKED_BANNER",
    ],
  };
  fs.writeFileSync(path.join(OUT, "phase11-remediation-v3-report.json"), JSON.stringify(report, null, 2), "utf8");
  fs.writeFileSync(
    path.join(OUT, "before-after-summary-v3.json"),
    JSON.stringify({ changedSurfaces, insufficientSourceContextCount, cases: beforeAfter }, null, 2),
    "utf8",
  );

  const checkpoint = `# Phase 11 remediation v3 checkpoint
...
| Insufficient source context | ${insufficientSourceContextCount} |

**v3 policy:** same membership retained for before/after comparison. Fixture repeats across strata are **justified** (multi-angle review), not silently altered.
...
`;
  ...
  ensureDir(DOCS);
  fs.writeFileSync(path.join(DOCS, "phase-11-remediation-v3-checkpoint.md"), checkpoint, "utf8");
  fs.writeFileSync(path.join(OUT, "PHASE-11-REMEDIATION-V3-CHECKPOINT.md"), checkpoint, "utf8");
  ...
  if (fs.existsSync(readmePath)) {
    ...
    if (readme.includes("Phase 11 —")) {
      readme = readme.replace(
        /\| Phase 11 —.*\|/,
        "| Phase 11 — rendered coverage + gold FP–FN | **REMEDIATION_V3_COMPLETE / AWAITING_HUMAN_GOLD_REVIEW** (v1+v2 preserved; v3 comparison) | `docs/integrity-programme/phase-11-remediation-v3-checkpoint.md` |",
      );
