...
| **B4** | Chat as case builder — propose summary/theory; Agree / Edit / Reject | ❌ | Not implemented; storage + edit UI exist for a future “propose → accept” flow |
...
- **B4** — Chat as case builder (propose summary/case theory from chat → Agree/Edit/Reject → write to agreed summary). Backing storage and edit UI exist; the “propose → accept” chat flow is not built.
- **Phase C (C1–C4)** — Police station spine: station as first phase, link when charged, dashboard one spine, chat + station grounding.
- **Change list driving plan updates** (D5 idea) — We record ratings + optional note; we don’t yet use that “change list” to auto-update plan/behaviour.
